...
|---|-------|-------------|--------|----------|---------|----------|
| 0 | Spike: Steam-on-iSCSI 사전 검증 | 수동으로 LIO iSCSI target 만들고 PC에서 마운트해 Steam 게임 1개 설치·실행. fio 벤치마크 baseline. 가설 사망 가능성 조기 탐지 | in-progress | - | - | [phase0-spike](../plans/network-storage-reframing-phase0-spike.plan.md) · [report](../../docs/spikes/phase0-steam-network-storage.md) |
| 1 | Storage SoT 재정의 ADR + schema 설계 | ADR 0003 작성. `drives`/`mount_credentials`/`share_grants` 스키마 초안. 기존 `files` deprecation 경로 명시 | pending | with 2 | 0 | - |
| 2 | Privileged storage agent (sidecar) 골격 | NAS 호스트 OS의 Samba/targetcli 관리를 위임받는 작은 데몬 (Go 또는 Rust). API↔agent는 unix socket | pending | with 1 | 0 | - |
| 3 | Web 콘솔: 1인용 발급/회수 + 가이드 다운로드 | "내 드라이브" + 마운트 발급 UI. `.ps1`/`.bat` 자동 생성. (가족 발급은 phase 5에서) | pending | - | 1, 2 | - |
...
*Generated: 2026-05-25 — Status: DRAFT (needs validation via Phase 0 spike + 가족 인터뷰)*
