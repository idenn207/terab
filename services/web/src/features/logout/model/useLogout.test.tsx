...
describe('useLogout', () => {
  beforeEach(() => {
    useUserStore.getState().setAuth('initial-token', { id: 'u1', username: 'testuser', nickname: 't' });
  });
  ...
  it('logout 성공 시 clearAuth 가 호출되어 store 가 초기화된다', async () => {
    server.use(
      http.post('/api/auth/logout', () => HttpResponse.json({}, { status: 200 })),
    );
  ...
  it('logout API 가 500 으로 실패해도 onSettled 에서 clearAuth 가 호출된다', async () => {
    server.use(
      http.post('/api/auth/logout', () => HttpResponse.json({ code: 'INTERNAL', message: 'oops' }, { status: 500 })),
    );
