# services/web/CLAUDE.md 작성 계획

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** `services/web/CLAUDE.md` 파일을 생성해 React + Capacitor Android 서비스의 FSD 레이어 규칙, 상태관리, 컴포넌트, API, Android 컨벤션, Claude 행동 지침을 문서화한다.

**Architecture:** 단일 CLAUDE.md 파일로 Web(React/FSD)과 Android(Capacitor) 컨벤션을 섹션으로 분리 관리한다. 루트 CLAUDE.md의 공통 원칙을 상속하고 web 전용 규칙만 추가 명시한다.

**Tech Stack:** Markdown — 별도 라이브러리 없음
...
| 파일 | 작업 | 역할 |
|------|------|------|
| `services/web/CLAUDE.md` | 신규 생성 | web 서비스 전용 Claude 행동 지침 |

---

### Task 1: Section 1–2 작성 (아키텍처 개요 + FSD 의존 규칙)

**Files:**
- Create: `services/web/CLAUDE.md`

- [ ] **Step 1: Section 1 (아키텍처 개요) 작성**

`services/web/CLAUDE.md` 파일을 신규 생성하고 아래 내용을 작성한다.
...
- [ ] **Step 2: Section 2 (FSD 레이어 의존 규칙) 추가**
...
```bash
wc -l services/web/CLAUDE.md
```

현재까지 약 45줄 예상. 300줄 제한 내인지 확인한다.

---

### Task 2: Section 3–4 추가 (컴포넌트 + 함수·export 컨벤션)

**Files:**
- Modify: `services/web/CLAUDE.md`

- [ ] **Step 1: Section 3 (컴포넌트 컨벤션) 추가**
...
- [ ] **Step 2: Section 4 (함수 선언 · export 컨벤션) 추가**
...
### Task 3: Section 5 추가 (상태관리 컨벤션)

**Files:**
- Modify: `services/web/CLAUDE.md`

- [ ] **Step 1: 상태관리 개요 + 핵심 규칙 추가**
...
### Task 4: Section 6–7 추가 (API 레이어 + Android/Capacitor)

**Files:**
- Modify: `services/web/CLAUDE.md`

- [ ] **Step 1: Section 6 (API 레이어 컨벤션) 추가**
...
- [ ] **Step 2: Section 7 (Android / Capacitor 컨벤션) 추가**
...
### Task 5: Section 8 추가 + 검증 + 커밋

**Files:**
- Modify: `services/web/CLAUDE.md`

- [ ] **Step 1: Section 8 (Claude 행동 지침) 추가**
...
예상: 240~270줄. 300줄 초과 시 상태관리 코드 예시 일부를 간소화한다.

- [ ] **Step 3: TBD / TODO / 플레이스홀더 검사**

```bash
grep -n "TBD\|TODO\|placeholder\|추후\|나중에" services/web/CLAUDE.md
```

예상: 결과 없음. 있으면 해당 줄을 확정 내용으로 교체한다.

- [ ] **Step 4: 커밋**

```bash
git add services/web/CLAUDE.md docs/superpowers/specs/2026-04-15-web-claude-md-design.md docs/superpowers/plans/2026-04-15-web-claude-md.md
git commit -m "docs: services/web CLAUDE.md 작성 — FSD·상태관리·Android 컨벤션"
```
