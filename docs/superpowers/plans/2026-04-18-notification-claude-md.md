# Notification CLAUDE.md Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** `services/notification/CLAUDE.md` 파일을 작성하여 notification 서비스 전용 개발 컨벤션을 문서화한다.

**Architecture:** 단일 파일 생성 작업. 스펙 문서(`docs/superpowers/specs/2026-04-18-notification-claude-md-design.md`)를 기반으로 실제 코드(PushEventConsumer, FcmPushService, application.yml, 테스트 파일)에서 검증한 패턴을 반영하여 작성한다.

**Tech Stack:** Markdown, Spring Cloud Stream, Firebase FCM, RabbitMQ, Testcontainers
...
| 작업 | 경로 |
|------|------|
| Create | `services/notification/CLAUDE.md` |

---

### Task 1: `services/notification/CLAUDE.md` 작성
...
아래 내용을 그대로 `services/notification/CLAUDE.md`에 작성한다.
...
Run: `ls services/notification/CLAUDE.md`
Expected: 파일 경로 출력 (오류 없음)

- [ ] **Step 3: 커밋**

```bash
git add services/notification/CLAUDE.md
git commit -m "docs: services/notification CLAUDE.md 작성"
```

Expected:
```
[feat/mobile-capacitor-design ...] docs: services/notification CLAUDE.md 작성
 1 file changed, N insertions(+)
 create mode 100644 services/notification/CLAUDE.md
```
...
| 스펙 섹션 | 대응 Task |
|----------|----------|
| 아키텍처 개요 (역할, 패키지 구조) | Task 1 Step 1 — 아키텍처 개요 섹션 |
| api 서비스와의 비교 표 | Task 1 Step 1 — 비교 표 |
| 주요 명령어 | Task 1 Step 1 — 주요 명령어 섹션 |
| Consumer Bean 네이밍 | Task 1 Step 1 — Consumer Bean 패턴 섹션 |
| application.yml 바인딩 규칙 | Task 1 Step 1 — 바인딩 규칙 표 |
| 새 이벤트 추가 체크리스트 | Task 1 Step 1 — 체크리스트 섹션 |
| 예외 처리 | Task 1 Step 1 — 예외 처리 섹션 |
| 테스트 구조 | Task 1 Step 1 — 테스트 컨벤션 섹션 |
| Javadoc · 주석 정책 | Task 1 Step 1 — Javadoc 섹션 |
| Claude 행동 지침 | Task 1 Step 1 — Claude 행동 지침 섹션 |

**플레이스홀더 스캔:** 없음.

**일관성 체크:** 단일 파일 생성이므로 타입 불일치 없음.
