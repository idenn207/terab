...
**Goal:** 프로젝트 루트에 `README.md`를 작성한다. clone 후 로컬 개발 환경 구성 → 운영 배포(NAS/Docker Swarm)까지 플로우 중심으로 구성하며, 트러블슈팅·설정 레퍼런스를 포함한다.

**Architecture:** 단일 파일(`README.md`). 읽는 순서 = 실행 순서. 트러블슈팅은 각 단계 바로 아래 인라인 테이블. 설정 레퍼런스는 별도 섹션에 분리.

**Tech Stack:** Markdown (GitHub 렌더링 기준), Mermaid (`graph LR`)
...
- [ ] **Step 1: README.md 생성 — 헤더, 배지, Mermaid 다이어그램 작성**

아래 내용을 `README.md`로 저장한다.
...
GitHub 또는 VSCode Markdown 미리보기에서 Mermaid 다이어그램이 정상 렌더링되는지 확인한다.  
배지 이미지가 깨지지 않는지 확인한다.

- [ ] **Step 3: 커밋**

```bash
git add README.md
git commit -m "docs: README 헤더 & 아키텍처 다이어그램 추가"
```
...
- [ ] **Step 1: 사전 요구사항 섹션을 README.md에 추가**

Task 1에서 작성한 내용 아래에 이어 붙인다.
...
```bash
git add README.md
git commit -m "docs: README 사전 요구사항 섹션 추가"
```

---

### Task 3: 로컬 개발 환경 구성 섹션

**Files:**
- Modify: `README.md`

- [ ] **Step 1: 로컬 개발 환경 구성 섹션을 README.md에 추가**

Task 2에서 작성한 내용 아래에 이어 붙인다.
...
`make setup-local` 실행 결과로 `services/api/application-local.properties`가 생성되는지 확인한다.

- [ ] **Step 3: 커밋**

```bash
git add README.md
git commit -m "docs: README 로컬 개발 환경 구성 섹션 추가"
```
...
- [ ] **Step 1: 운영 배포 섹션을 README.md에 추가**

Task 3에서 작성한 내용 아래에 이어 붙인다.
...
```bash
git add README.md
git commit -m "docs: README 운영 배포 섹션 추가"
```
...
> **주의:** `application.yml`에 `cors_allowed_origins` 키가 존재하나 `configs.env.example`에 누락되어 있다. 레퍼런스 테이블에 추가하고, `configs.env.example`도 함께 수정한다.

- [ ] **Step 1: 설정 레퍼런스 섹션을 README.md에 추가**

Task 4에서 작성한 내용 아래에 이어 붙인다.
...
- [ ] **Step 2: configs.env.example에 cors_allowed_origins 추가**

`configs.env.example` 파일 마지막에 아래 줄을 추가한다.
...
```bash
git add README.md configs.env.example
git commit -m "docs: README 설정 레퍼런스 섹션 추가, configs.env.example cors 키 누락 수정"
```
...
- [ ] **Step 1: 프로젝트 구조 섹션을 README.md에 추가**

Task 5에서 작성한 내용 아래에 이어 붙인다.
...
GitHub 또는 VSCode Markdown 미리보기에서 전체 README를 위에서 아래로 읽으며 확인한다:
- Mermaid 다이어그램 렌더링
- 모든 표가 정렬됨
- 코드 블록 언어 지정 정확
- 앵커 링크(`#설정-레퍼런스`) 작동
...
```bash
git add README.md
git commit -m "docs: README 프로젝트 구조 섹션 추가 — README 완성"
```
...
| 스펙 요구사항 | 구현 태스크 |
|---|---|
| 헤더 & 소개, Mermaid 다이어그램 | Task 1 |
| 사전 요구사항 (로컬/운영), 외부 링크 | Task 2 |
| 로컬 개발 흐름, make 명령어 표, 트러블슈팅 | Task 3 |
| 운영 배포 흐름, Config/Secret 개념, 트러블슈팅 | Task 4 |
| 설정 레퍼런스 테이블 (configs / secrets) | Task 5 |
| 프로젝트 구조 디렉토리 트리 | Task 6 |
| cors_allowed_origins 누락 수정 | Task 5 Step 2 |

**Placeholder 스캔:** 없음. 모든 Markdown 콘텐츠가 완성된 형태로 포함됨.

**일관성 체크:** 모든 태스크에서 `make` 명령어 이름이 Makefile 정의와 일치. 포트 번호(5432, 9000, 9001, 8080, 5173) 일관됨. 설정 키 이름이 `application.yml` 및 `configs/secrets.env.example`과 일치.
