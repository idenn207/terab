...
await RequestTraceContext.run(
  { reqId: workerJobId, userId: null, route: 'worker:cleanup', spans: [], startedAt: Date.now() },
  async () => {
...
| 경로 | 역할 |
|---|---|
| `src/logger/trace/request-trace-context.ts` | CLS 기반 요청 컨텍스트 |
| `src/logger/trace/service-method-wrapper.ts` | `ServiceCore` 자손 자동 wrap |
| `src/logger/trace/trace.interceptor.ts` | 글로벌 NestJS 인터셉터 |
| `src/logger/trace/trace.flusher.ts` | meta/detail 직렬화 + pino 위임 |
| `src/logger/trace/trace.limits.ts` | 버퍼 한계 상수 |
| `src/logger/trace/decorators/auto-trace.decorator.ts` | `@AutoTrace()` |
| `src/logger/trace/decorators/log-replay.decorator.ts` | `@LogReplay()` |
| `src/logger/trace/decorators/skip-trace.decorator.ts` | `@SkipTrace()` |
| `src/logger/trace/pii-masker.ts` | 키 블랙리스트 마스킹 |
| `src/logger/trace/drizzle-query-logger.ts` | drizzle `logger.logQuery` 콜백 |
| `src/logger/trace/*.spec.ts` | 각 컴포넌트 단위 테스트 |
