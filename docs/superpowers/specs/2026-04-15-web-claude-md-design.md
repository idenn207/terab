# services/web/CLAUDE.md 설계 문서

**날짜:** 2026-04-15
**범위:** `services/web/` (React 웹 + Capacitor Android)
**최종 파일:** `services/web/CLAUDE.md` (200줄 이내, 필요 시 300줄까지)

---

## 배경 및 목적

`services/web/`은 React 19 + Vite 기반 웹 앱과 Capacitor 8 기반 Android 앱을 함께 관리한다.
루트 `CLAUDE.md`의 공통 원칙을 상속하되, FSD 레이어 구조·상태관리·Android 네이티브 플러그인 등
web 전용 컨벤션을 Claude가 일관되게 적용할 수 있도록 명시한다.
...
| 항목               | 결정                                                | 근거                                |
| ------------------ | --------------------------------------------------- | ----------------------------------- |
| 파일 구조          | 단일 파일, Web·Android 섹션 분리                    | 같은 빌드 파이프라인(cap:sync) 공유 |
| FSD 레이어         | app·pages·widgets·features·entities·shared          | widgets 레이어 존재 확인            |
| FSD 의존 방향      | 엄격 적용, 표로 명시                                | 위반 시 설계 검토 필요              |
| pages/share 명칭   | 공유(Share) 도메인 페이지, FSD shared 레이어와 무관 | 혼동 방지                           |
| Android 상세도     | 보통 + 네이티브 플러그인 가이드 포함                | 대용량 업로드·백그라운드 기능 예정  |
| 플러그인 위치      | 인라인 (android/app/src/main/java/…/plugins/)       | 초기 규모에 적합                    |
| API 인스턴스       | shared/api/axiosInstance.ts 단일 인스턴스           | 401 refresh queue 내장              |
| 함수 선언          | 최상위 function, 내부 arrow function                | this 바인딩 일관성                  |
| export 패턴        | .tsx 즉시 export, .ts 하단 일괄 export              | 기존 코드 패턴 일치                 |
| 상태관리 위반 검사 | 6가지 항목 순서대로 검사                            | Claude 행동 지침에 포함             |
...
- `features/upload` → `features/login` import 금지
- 공통 로직은 `shared/`로 내려야 한다
...
동일한 데이터를 두 곳에 저장하지 않는다. 파생 가능한 값은 상태로 만들지 않고 계산한다.
...
서버에서 온 데이터(`user`, `files`)와 UI 제어 값(`isModalOpen`, `selectedTab`)을 같은 스토어에 혼재하지 않는다.
서버 상태는 `entities/` 레이어 스토어, UI 상태는 컴포넌트 내 `useState` 또는 `features/` 모델로 분리한다.

**공유 상태 최소화**

Zustand 스토어는 진짜 전역이 필요한 경우에만 사용한다. 단일 컴포넌트 트리 내에서만 쓰이는 상태는 `useState`로 로컬 관리하고, 스토어 수를 늘리기 전에 props 전달로 해결 가능한지 먼저 검토한다.
...
### Section 7 — Android / Capacitor 컨벤션

Web(React) 빌드 결과물을 Capacitor가 Android WebView로 감싸는 구조다.
네이티브 기능은 Capacitor 플러그인 방식으로 `android/` 내에 인라인으로 작성한다.
...
`capacitor.config.ts`의 `server` 블록 주석 해제 후 `cap:sync` 실행.
로컬 개발 중에는 해당 블록을 주석 상태로 유지한다.
...
| 섹션                           | 예상 줄 수 |
| ------------------------------ | ---------- |
| Section 1 아키텍처 개요        | ~30        |
| Section 2 FSD 레이어 의존 규칙 | ~25        |
| Section 3 컴포넌트 컨벤션      | ~10        |
| Section 4 함수 선언·export     | ~20        |
| Section 5 상태관리             | ~70        |
| Section 6 API 레이어           | ~8         |
| Section 7 Android/Capacitor    | ~40        |
| Section 8 Claude 행동 지침     | ~40        |
| **합계**                       | **~243줄** |
